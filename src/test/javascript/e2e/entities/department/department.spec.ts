...
describe('Department e2e test', () => {
    ...
    it('should create and save Departments', async () => {
        ...
        await departmentComponentsPage.clickOnCreateButton();
        await promise.all([departmentUpdatePage.setNameInput('name'), departmentUpdatePage.setShortNameInput('shortName')]);
        expect(await departmentUpdatePage.getNameInput()).to.eq('name');
        expect(await departmentUpdatePage.getShortNameInput()).to.eq('shortName');
